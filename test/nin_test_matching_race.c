...
{
  fprintf(stderr, "Usage: ./matching_race <type: 0=SR, 1=SSR> <matching safe: 0=unsafe 1=safe > <# of loops> <# of patterns per loop> <interval(usec)>\n");
  return;
}
...
static int* nin_convert_dest_flags_to_dests(int scount, int *dest_flags)
{
  ...
  for (dest = 0; dest < size; dest++) {
    if (dest_flags[dest]) { 
      //      mst_test_dbg_print("dest: %d", dest);
      dests[index++] = dest;
    }
  }
  return dests;
...
static void ssr(int num_patterns, int interval_usec, int loop_count, int is_safe)
{
  ...
  for (i = 0; i != loop_count; i++) {
	...
	recv_count = nin_allreduce_recv_counts(dest_flags);      
	//	mst_test_dbg_print("count: %d", recv_count);
